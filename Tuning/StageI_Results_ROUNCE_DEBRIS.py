...
MF = params[:,3]
# 
mb0_4999 = np.loadtxt('D:/TuningOutputs/ROUNCE_DEBRIS/StageI/mb_results_0-4999.csv',delimiter=',')
mb5000_9999 = np.loadtxt('D:/TuningOutputs/ROUNCE_DEBRIS/StageI/mb_results_5000-9999.csv',delimiter=',')

mb = np.concatenate((mb0_4999,mb5000_9999),axis=0)
...
df.to_csv('D:/TuningOutputs/ROUNCE_DEBRIS/StageI/Tuning_Params_StageI_' + model_name + '.csv')

d2 = {'simID': simID_passing, 'MB': mb_passing}
df2 = pd.DataFrame(data=d2)
df2.to_csv('D:/TuningOutputs/ROUNCE_DEBRIS/StageI/Tuning_Params_StageI_MB_' + model_name + '.csv')
...
# =============================================================================
# Load results from ref model in order to compare
# =============================================================================
...
mb_refmodel = np.concatenate((mb0_1000,mb1001_2000,mb2001_3000,mb3001_4000,mb4001_4999,mb5000_5999,mb6000_6999,mb7000_7999,mb8000_8999,mb9000_9999),axis=0)

simID_refmodel = np.arange(0,len(mb_refmodel))  

mb_aice_geq_asnow_refmodel = np.delete(mb_refmodel,np.where(aice<asnow))
simID_aice_geq_asnow_refmodel = np.delete(simID_refmodel,np.where(aice<asnow))

simID_passing_refmodel = simID_aice_geq_asnow_refmodel[np.where((mb_aice_geq_asnow_refmodel >= (target_mb-(3*tagret_mb_stddev))) & (mb_aice_geq_asnow_refmodel <= (target_mb+(3*tagret_mb_stddev))))]
mb_passing_refmodel = mb_aice_geq_asnow_refmodel[np.where((mb_aice_geq_asnow_refmodel >= (target_mb-(3*tagret_mb_stddev))) & (mb_aice_geq_asnow_refmodel <= (target_mb+(3*tagret_mb_stddev))))]
...
plt.xticks(np.arange(-11.5,1.1,0.5),np.arange(-11,1.1,0.5),fontsize=14,rotation=45)
...
plt.figure(figsize=(12,5))
plt.title('All runs\nTotal = ' + str(len(mb)),fontsize=14)
plt.hist(mb_refmodel,mb_bins,rwidth=1,color='red',zorder=5,alpha=0.5,label='REF_MODEL')
plt.hist(mb,mb_bins,rwidth=1,color='mediumblue',zorder=5,alpha=0.5,label='ROUNCE_DEBRIS')
plt.hist(mb_refmodel,mb_bins, histtype='step', stacked=True, fill=False,color='red',linewidth=3,zorder=10)
plt.hist(mb,mb_bins, histtype='step', stacked=True, fill=False,color='mediumblue',linewidth=3,zorder=10)
plt.ylabel('Frequency',fontsize=14)
plt.xlabel('2007-2018 Mass Balance (m w.e. $a^{-1}$)',fontsize=14)
plt.xticks(np.arange(-11.5,1.1,0.5),np.arange(-11,1.1,0.5),fontsize=14,rotation=45)
plt.yticks(fontsize=14)
...
plt.figure(figsize=(12,5))
plt.title('All runs where a$_{ice}$ $\geq$ a$_{snow}$\nTotal = ' + str(len(mb_aice_geq_asnow)),fontsize=14)
plt.hist(mb_aice_geq_asnow_refmodel,mb_bins,rwidth=1,color='red',zorder=5,alpha=0.5,label='REF_MODEL')
plt.hist(mb_aice_geq_asnow,mb_bins,rwidth=1,color='mediumblue',zorder=5,alpha=0.5,label='ROUNCE_DEBRIS')
plt.hist(mb_aice_geq_asnow_refmodel,mb_bins, histtype='step', stacked=True, fill=False,color='red',linewidth=3,zorder=10)
...
# =============================================================================
mb_normaldist_bins =  np.linspace(target_mb-(3*tagret_mb_stddev),target_mb+(3*tagret_mb_stddev),int(np.sqrt(len(mb_passing)))+1)
#mb_normaldist_bins =  np.linspace(target_mb-(3*tagret_mb_stddev),target_mb+(3*tagret_mb_stddev),26)
...
plt.figure(figsize=(12,5))
plt.grid(zorder=20)
plt.title(model_name + ': Target MB $\pm$ 3$\sigma$\nTotal = ' + str(len(mb_passing)),fontsize=14)
...
plt.figure(figsize=(12,5))
plt.grid(zorder=20)
plt.title(model_name + ': ' + str(len(mb_passing)) + ' sims\nREF_MODE: ' + str(len(mb_passing_refmodel)) + ' sims',fontsize=14)
plt.hist(mb_passing_refmodel,mb_normaldist_bins,rwidth=0.9,color='red',zorder=5,alpha=1,label='REF_MODEL')
plt.hist(mb_passing,mb_normaldist_bins,rwidth=0.9,color='mediumblue',zorder=5,alpha=1,label='ROUNCE_DEBRIS')
plt.ylabel('Frequency',fontsize=14)
plt.xlabel('2007-2018 Mass Balance (m w.e. $a^{-1}$)',fontsize=14)
plt.yticks(fontsize=14)
#plt.xticks(np.arange(-0.98,0.12,0.04),np.round(np.arange(-0.98,0.12,0.04),2),fontsize=14,rotation=45)
...
set1 = set(simID_passing_refmodel)
set2 = set(simID_passing)
